...
public class OglasServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		List<Ogla> oglasi = bean.searchOglas(text);
		System.out.println("Oglasi iz doGet .size() "+ oglasi.size());
		
		
		if(oglasi == null) {
			request.setAttribute("message", "User likely null.");
			request.getRequestDispatcher("error.jsp").forward(request, response);
		}else if(oglasi.isEmpty()) {
			request.setAttribute("message", "NEMA BRT.");
			request.getRequestDispatcher("error.jsp").forward(request, response);
		}else {
			request.setAttribute("oglasi", oglasi);
			request.getRequestDispatcher("javljanje-form.jsp").forward(request, response);
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		boolean succ = bean.createOglas(text);
		if (succ) {
			request.setAttribute("message", "Uspesno dodao oglas.");
